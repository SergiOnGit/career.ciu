...
class Page extends CI_Controller {
	...
	function __construct() {
		parent::__construct();
		$this->load->model(array('Front_model', 'Front_page_model'));
	}

	public function index($slug = null)
	{
		if($slug === null) redirect(base_url());
		$data['data'] = $this->Front_page_model->getBySlug(urldecode($slug));

		if($data['data']['slug_'.LANG] != urldecode($slug)) {
				redirect(site_url('/page/index/'.$data['data']['slug_'.LANG].'?lang='.LANG));
		}
