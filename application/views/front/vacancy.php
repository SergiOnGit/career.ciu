    ...
    <div class="wrap news-page">
        <div class="pages-header-wrap">
            <ul class="uk-breadcrumb">
                ...
                <li><a href="/all_vacancy">ყველა ვაკანსია</a></li>
                            ...
                            <?php if (count($data['category']) && !empty($data['category'])): ?>
                                    <div class="news-card-tags-wrap">
                                        ...
                                        <?php  foreach ($data['category'] as $value): ?>
                                            ...
                                            <a href="/all_vacancy?cat=<?=$value['id']?>" class="news-card-tags" style="color: #000;">
                                                <?=$value['title_'.LANG]?>
                                            </a>
                                        <?php endforeach ?>
                                    </div>
            ...
            <div class="uk-width-medium-3-10">
                <div class="related-news">
                    <header class="block-header">
                        <h3 class="block-header-title">ვაკანსიები</h3>
                        <div class="block-header-actions">
                            <a href="<?=base_url('all_vacancy')?>" class="block-header-btn">ყველა</a>
                        </div>
                    </header>
                    <div>
                        <?php foreach ($latest_vacancy as $value): ?>
                        <div class="announces-card">
                            <div class="announces-card-date">
                                <span class="news-date"><?=$value['date']?></span>
                            </div>
                            <h3 class="announces-card-title">
                                <a href="<?=base_url('vacancy/index/').$value['slug_'.LANG]?>"><?=$value['title_'.LANG]?></a>
                            </h3>
                            <div class="news-card-tags-wrap">
                                <?php foreach($value['categories'] as $row): ?>
                                    <a href="<?=base_url()?>all_vacancy?cat=<?=$row['id']?>" class="news-card-tags" style="color: #<?=$row['color']?>;">
                                        <?=$row['title_'.LANG]?>
                                    </a>
                                <?php endforeach; ?>
                            </div>
